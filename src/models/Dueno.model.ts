...
export const createDueno = async (DuenoData: DuenoData): Promise<void> => {
  ...
  await pool.query(
    'INSERT INTO duenos (name, lastname, adress, phone) VALUES (?, ?, ?, ?)',
...
export const getDuenoById = async (id: number): Promise<Dueno | null> => {
  const [rows] = await pool.query<DuenoRow[]>(
    'SELECT id, name, lastname, adress, phone FROM duenos WHERE id = ?',
    [id]
  );
...
export const updateDueno = async (id: number, data: Partial<Dueno>): Promise<void> => {
  ...
  await pool.query(
    'UPDATE duenos SET name = ?, lastname = ?, adress = ?, phone = ? WHERE id = ?',
